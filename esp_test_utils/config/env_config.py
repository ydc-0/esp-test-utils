...
class EnvConfig:
    ...
    Example usage:
        ...
        ```python
        env_config = EnvConfig(env_name='my_env')
    ...
    def __init__(self, env_name: str = 'default', config_file: Optional[str] = None) -> None:
        ...
        if self.config_file:
            if os.path.isfile(self.config_file):
                ...
                assert env_name in raw_data
                assert isinstance(raw_data[env_name], dict)
                self.config_data = raw_data[env_name]
    ...
    def get_variable(self, key: str, default: Any = None) -> Any:
        ...
        if key in self.config_data:
            ...
            logging.warning(f'Failed to get env variable {self.env_name}/{key}.')
            logging.info(self.__doc__)
            if not self.ALLOW_INPUT:
                raise ValueError(f'Env variable not found: {self.env_name}/{key}')
            # For local test, support input the variable from console
            var = input('You can input the variable now:')
        logging.debug(f'Got env variable {self.env_name}/{key}: {var}')
        return var
