...
class BasePort(metaclass=abc.ABCMeta):
...
class PortSpawn(pexpect.spawnbase.SpawnBase):
    ...
    def __init__(
        ...
        Args:
            port (BasePort): port instance with read() method.
...
class PortWrapper:
    """Simply all devices / shell / sockets to similar usage"""
    ...
    def expect(
        self,
        pattern: Union[str, bytes, re.Pattern],
        timeout: Union[int, float] = PEXPECT_DEFAULT_TIMEOUT,
    ) -> Optional[re.Match]:
