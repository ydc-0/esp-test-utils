...
from esp_test_utils.basic import NetTools
...
def test_get_local_ip_by_interface(patch_psutil_addrs):
    patch_psutil_addrs.return_value = MOCK_NETIF_ADDRS
    ip = NetTools.get_ip4_from_interface('lo')
    assert ip == '127.0.0.1'
    ip = NetTools.get_ip6_from_interface('lo')
    assert ip == '::1'
    assert patch_psutil_addrs.call_count > 0


@mock.patch('psutil.net_if_addrs')
def test_guess_ipv6(patch_psutil_addrs):
    patch_psutil_addrs.return_value = MOCK_NETIF_ADDRS
    iter_ip = NetTools.guess_local_ip6('fe80::1')
    ip = next(iter_ip)
    assert ip == r'fe80::2%if1'


@mock.patch('psutil.net_if_addrs')
def test_netif_vs_mac(patch_psutil_addrs):
    patch_psutil_addrs.return_value = MOCK_NETIF_ADDRS
    interface = NetTools.get_interface_by_mac('11:22:33:44:55:66')
    assert interface == 'if1'
    mac = NetTools.get_mac_by_interface('if1')
